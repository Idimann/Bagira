...
const pi2 = @import("movepick2.zig");
const hi2 = @import("history2.zig");
...
pub const Searcher = struct {
    ...
    pub fn init(b: *bo.Board, alloc: std.mem.Allocator) Searcher {
        return .{
            .alloc = alloc,
            .b = b,
            .start_ply = b.hash_in,
            .stats = hi2.Stats.init(),
        };
    }
    ...
    pub fn search(self: *Searcher, bound: Prob, until_prob: Prob) !BoundProb {
        ...
        // TT Probe
        const tte = tt2.probe(self.b);
        const tte_fine = tte.reader != null and tte.usable;

        // Trust the tt entry if it's usable
        if (tte_fine and tte.reader.?.usable(bound)) return tte.reader.?.val.score;
        ...
        var pick = pi2.Picker.init(.TT, self, &gen, tte);
        defer pick.deinit();
